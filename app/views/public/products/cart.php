            ...
            <?php foreach ($cartItems as $item): ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4 p-4 border-b dark:border-gray-800 items-center hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                    ...
                    <div class="col-span-4 md:col-span-2 text-center flex items-center justify-center gap-2">
                        ...
                        <button class="btn-decrease bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 px-3 py-1 rounded font-bold transition-colors"
                                data-key="<?= $cartKey ?>">-</button>

                        <input type="number"
                            value="<?= $item->quantity ?>"
                            min="1"
                            class="quantity-input w-14 text-center p-1 border rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-yellow-500 focus:border-yellow-500"
                            data-key="<?= $cartKey ?>">

                        <button class="btn-increase bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 px-3 py-1 rounded font-bold transition-colors"
                                data-key="<?= $cartKey ?>">+</button>
                    </div>
                ...
                <div class="text-right">
                    <div class="mt-8 bg-gray-100 dark:bg-gray-800 p-6 rounded-lg">
                        ...
                        <form action="<?= URLROOT ?>/public/index.php?url=products/checkout" method="POST" class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 text-left">Họ và tên</label>
                                <input type="text" name="fullname" required class="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-yellow-500 focus:border-yellow-500">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 text-left">Số điện thoại</label>
                                <input type="text" name="phone" required class="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-yellow-500 focus:border-yellow-500">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 text-left">Địa chỉ nhận hàng</label>
                                <input type="text" name="address" required class="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-yellow-500 focus:border-yellow-500">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 text-left">Ghi chú</label>
                                <textarea name="note" class="w-full p-2 border rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-yellow-500 focus:border-yellow-500"></textarea>
                            </div>
...
function updateQuantity(key, quantity) {
    ...
    .then(data => {
        if (data.success) {
            document.getElementById("total-" + key).innerText = new Intl.NumberFormat().format(data.itemTotal) + " đ";
            document.getElementById("cart-grand-total").innerText = new Intl.NumberFormat().format(data.cartTotal) + " đ";
